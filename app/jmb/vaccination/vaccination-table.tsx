  ...
  return (
    <main className="min-h-[calc(100vh-4rem)] pb-10">
      ...
      <section className="m-3 mt-6 overflow-hidden rounded-xl border bg-card shadow-sm">
        ...
        <Table><TableHeader><TableRow><TableHead className="w-14 text-center">#</TableHead><TableHead>Scheduled Date</TableHead><TableHead>Actual Date</TableHead><TableHead>Variance (days)</TableHead><TableHead>Document</TableHead><TableHead>Farm / location</TableHead><TableHead>Vaccine</TableHead><TableHead>Disease</TableHead><TableHead>Batch</TableHead><TableHead>Birds</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader><TableBody>
          {loading ? <TableRow><TableCell colSpan={12} className="h-32 text-center"><Loader2 className="mx-auto size-5 animate-spin" /></TableCell></TableRow> : null}
          {!loading && filtered.map((row, index) => <TableRow key={row.id} className={row.status === "Cancelled" ? "opacity-60" : ""}><TableCell className="text-center font-medium tabular-nums text-muted-foreground">{index + 1}</TableCell><TableCell>{row.scheduled_vaccination_date ? formatDate(row.scheduled_vaccination_date) : "—"}</TableCell><TableCell>{formatDate(row.vaccination_date)}</TableCell><TableCell>{row.date_variance_days == null ? "—" : row.date_variance_days > 0 ? `+${row.date_variance_days}` : row.date_variance_days}</TableCell><TableCell className="font-mono text-xs">{row.document_no}</TableCell><TableCell><div className="font-medium">{row.farm_name}</div><div className="max-w-64 truncate text-xs text-muted-foreground">{row.scope}: {row.target_names || row.building_name || row.farm_name}</div></TableCell><TableCell><div className="font-medium">{row.vaccine_brand}</div><div className="text-xs text-muted-foreground">{row.vaccine_type} · {Number(row.dosage).toLocaleString("en-US")} · {row.route}</div></TableCell><TableCell>{row.disease_target}</TableCell><TableCell><div className="font-mono text-xs">{row.batch_number}</div><div className="text-xs text-muted-foreground">Exp {formatDate(row.expiry_date)}</div></TableCell><TableCell><div className="tabular-nums">{row.birds_vaccinated == null ? "—" : Number(row.birds_vaccinated).toLocaleString()}</div><div className="text-xs text-muted-foreground">{row.birds_missed == null ? "" : `${row.birds_missed} missed`}</div></TableCell><TableCell><span className={`rounded-full px-2 py-1 text-xs font-medium ${row.status === "Posted" ? "bg-emerald-100 text-emerald-700" : "bg-stone-200 text-stone-600"}`}>{row.status}</span></TableCell><TableCell><div className="flex justify-end gap-2"><Button type="button" size="sm" variant="outline" onClick={() => router.push(`/jmb/vaccination/new?id=${row.id}`)} className="border-emerald-700 text-emerald-700 hover:bg-emerald-50 hover:text-emerald-800"><Pencil className="size-4" />Edit/View</Button>{row.status === "Posted" ? <Button size="sm" variant="outline" className="text-red-600" onClick={() => { setCancelRecord(row); setCancelReason(""); }}><Ban className="size-4" />Cancel</Button> : null}</div></TableCell></TableRow>)}
          {!loading && !filtered.length ? <TableRow><TableCell colSpan={12} className="h-32 text-center text-muted-foreground">No vaccination records found.</TableCell></TableRow> : null}
