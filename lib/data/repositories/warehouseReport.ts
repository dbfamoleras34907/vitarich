...
const DOCUMENT_CONFIG: Record<string, DocumentConfig> = {
  ...
  BR_DELIVERY_REVERSAL: { table: 'br_delivery', numberColumn: 'gi_no', url: id => `/brd/dr/post?id=${id}` },
  ...
  BR_CLEANUP_REVERSAL: { table: 'br_cleanup', numberColumn: 'gi_no', url: id => `/brd/cu/post?id=${id}` },
  BR_CLEANUP_VARIANCE_REVERSAL: { table: 'br_cleanup', numberColumn: 'gi_no', url: id => `/brd/cu/post?id=${id}` },
}
